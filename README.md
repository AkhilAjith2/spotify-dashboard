...
First, open the `spotify-dashboard-main` directory with VS Code. Make sure you're in the correct directory (`spotify-dashboard-main`), then create a virtual environment:
...
3. **Important:** Select the venv kernel from the notebook's kernel selector (top-right corner). This should be the virtual environment you created in step 1. After selecting the venv, please restart VS Code before running the cells. Activate the venv again using `venv\Scripts\activate` for Windows and `source venv/bin/activate` for macOS/Linux.
